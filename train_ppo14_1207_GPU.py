#控制末端最后一个点到指定位置，1206根据佳煜师兄指导修改，1207修改到GPU
import torch
from stable_baselines3 import PPO
from nn_environment6_circle import NNEnvironment  # 使用更新后的环境代码
...
def create_env():
    ...
    nn_model.load_state_dict(torch.load('nn_models/trained_nn_model10_swgt_fixRseed_centered.pth',  map_location=device, weights_only=True))
    nn_model.to(device)  # 将模型移到 GPU
    nn_model.eval()
    # 创建环境的实例
...
class CustomCallback(BaseCallback):
    ...
    def _on_training_start(self):
        if self.writer is None:
            self.writer = SummaryWriter('runs/ppo_1211_circle_2')
...
if __name__ == '__main__':
    ...
    # 初始化并行环境
    env = SubprocVecEnv([lambda: create_env() for _ in range(40)])
    # 创建 PPO 模型，设置 batch_size 为 128
    model = PPO('MlpPolicy', env, verbose=1, batch_size=128, tensorboard_log="./ppo_tensorboard/", device=device)
    # # 创建回调
    # callback = CustomCallback()
    # 创建回调并设置保存路径和频率
    callback = CustomCallback(save_path="ppo_1211_circle_2/checkpoints", save_frequency=10)
    # 开始训练
    model.learn(total_timesteps=4000000, callback=callback)
    # 保存模型
    model.save("ppo_1211_circle_2")
