...
class NNEnvironment(gym.Env):
    ...
    def __init__(
        ...
        print(f"[Env {env_index}] Using device: {self.device}")
        ...
        # === 改动：不再自己加载模型，不再调用 .to() / .eval()。而是直接用外部传进来的 nn_model
        # self.nn_model = nn_model.to(self.device)
        # self.nn_model.eval()
        self.nn_model = nn_model  # 仅保存引用

        # ============== 压力限制 ==============
        self.pressure_low = np.array([0, 0, 0], dtype=np.float32)
    ...
    def step(self, action: np.ndarray):
        ...
        with torch.no_grad():
            pressure_tensor = torch.tensor(
                self.current_pressure,
                dtype=torch.float32
            ).unsqueeze(0).to(self.device)  # 加到 self.device 上
            new_state_tensor = self.nn_model(pressure_tensor)  # 直接调用传入的模型引用
    ...
    def reset(self, *, seed=None, options=None):
        ...
        # 1) 重置气压、步数、时间步 t
        self.current_pressure = np.array([0.0, 0.0, 0.0], dtype=np.float32)
        self.steps = 0
        self.t = 0
        self.previous_distance_to_target = None
        ...
        # 4) 拼装观察量
        future_points = self._get_future_trajectory(self.t, end_effector_position, horizon=10)
        observation = np.concatenate([
            keypoints,
            self.current_pressure,
            future_points
