...
class NNEnvironment(gym.Env):
    def __init__(self, nn_model, device='cpu'):
        ...
        self.action_space = gym.spaces.Box(low=-6, high=6, shape=(3,), dtype=np.float32)
    ...
    def step(self, action):
        ...
        # 时间惩罚：仅当 distance_to_target > 0.2 时才计算
        if distance_to_target > 0.2:
            time_penalty = -0.01 * self.steps  # 线性时间惩罚
        else:
            time_penalty = 0  # 小于 0.2 时不惩罚
        # 或者使用指数惩罚
        # time_penalty = -np.exp(0.01 * self.steps)

        total_reward = distance_reward + close_to_target_reward + termination_penalty + time_penalty
        ...
        info = {
            ...
            'time_penalty': time_penalty  # 将 time_penalty 添加到 info 中
        }
